...
export function useURLFilters(): [CommentLetterFilters, (updates: Partial<CommentLetterFilters>) => void] {
  ...
  const filters: CommentLetterFilters = useMemo(() => ({
    ...
    publicOnly: searchParams.get('public') === 'false' ? false : true,
  ...
  const setFilters = useCallback((updates: Partial<CommentLetterFilters>) => {
    setSearchParams(prev => {
      ...
      const nonPageKeys = ['search', 'topic', 'year', 'letterType', 'industry', 'publicOnly', 'sort'] as const;
      ...
      for (const [key, value] of Object.entries(updates)) {
        ...
        if (value === undefined || value === '' || value === null) {
          next.delete(param);
        } else if (key === 'publicOnly') {
          // Only set URL param when false (true is the default, keep URL clean)
          if (value === false) {
