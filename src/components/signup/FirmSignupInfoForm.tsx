...
interface FirmSignupInfoFormProps {
  selectedProduct: string;
  onBack: () => void;
  onSubmit: (data: FirmSignupFormData) => void;
  isLoading: boolean;
}

export interface FirmSignupFormData {
  firstName: string;
  lastName: string;
  email: string;
  company: string;
...
export function FirmSignupInfoForm({ selectedProduct, onBack, onSubmit, isLoading }: FirmSignupInfoFormProps) {
  const [formData, setFormData] = useState<FirmSignupFormData>({
    firstName: "",
    lastName: "",
    email: "",
    company: "",
  ...
  const [errors, setErrors] = useState<Partial<Record<keyof FirmSignupFormData, string>>>({});

  const product = FIRM_STRIPE_PRODUCTS[selectedProduct as keyof typeof FIRM_STRIPE_PRODUCTS];

  const validateForm = (): boolean => {
    const newErrors: Partial<Record<keyof FirmSignupFormData, string>> = {};
    ...
    if (!formData.company.trim()) {
      newErrors.company = "Firm name is required";
    }
  ...
  const handleChange = (field: keyof FirmSignupFormData) => (
  ...
  // Generate user count options (1-50)
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2">
          <Card className="shadow-xl border-border/50 bg-card backdrop-blur-sm">
            <CardHeader className="pb-4">
              <CardTitle className="text-2xl">Complete Your Signup</CardTitle>
              <p className="text-muted-foreground">
                Enter your firm details to continue to payment
              </p>
            </CardHeader>
            <CardContent>
        ...
        <div className="lg:col-span-1">
          <Card className="shadow-xl border-border/50 bg-card backdrop-blur-sm sticky top-24">
            <CardHeader className="pb-4">
